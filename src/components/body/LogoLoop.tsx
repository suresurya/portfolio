...
export type LogoNodeItem = LogoBase & {
  node: ReactNode;
};

export type LogoImageItem = LogoBase & {
  src: string;
  alt?: string;
  srcSet?: string;
  sizes?: string;
...
export const LogoLoop = memo(
  ({
    ...
    const sharedDeps = useMemo(() => [logos, gap, logoHeight, isVertical], [logos, gap, logoHeight, isVertical]);

    useResizeObserver(updateDimensions, observedElements, sharedDeps);
    useImageLoader(seqRef, updateDimensions, sharedDeps);
    ...
    const containerStyle = useMemo<LogoLoopStyle>(
      () => ({
        width: isVertical ? (resolvedWidth === "100%" ? undefined : resolvedWidth) : (resolvedWidth ?? "100%"),
        ...cssVariables,
        ...style,
      }),
      [resolvedWidth, cssVariables, style, isVertical]
    );
